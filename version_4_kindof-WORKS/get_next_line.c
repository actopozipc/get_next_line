...
/*   Updated: 2024/01/12 17:18:56 by lbaumeis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (buf != NULL)
	{
		...
		if (next && ft_strchr(temp, '\n') >= 0)
			return (buf = ft_duplicate(temp, ft_strchr(temp, '\n') + 1, -1), next);
		buf = ft_buf(fd);
	}
	if (!next)
...
///variable for strchr >> 2 lines more but then it saves space and still < 25lines and also not more than 80 colums wide!!